...
extern "C" {
	...
	JNIEXPORT void JNICALL Java_com_as_imgpro_arbrands_OpenCV_extractSURFFeature(JNIEnv* env, jobject thiz) {
		...
		CvSURFParams params = cvSURFParams(mSURFParams, 0);
		cvExtractSURF( pWorkImage, 0, &imageKeypoints, &imageDescriptors, storage, params );

		LOGI("Total ImageDescriptors : %d " , imageDescriptors->total );
		LOGI("Total ImageKeypoints : %d " , imageKeypoints->total );
		...
		for( int i = 0; i < imageKeypoints->total; i++ )
		{
			...
			LOGI("(%f, %f) : Size = %f, Dir = %f, hessian = %f" , r->pt.x, r->pt.y, r->size, r->dir, r->hessian  );
	...
	JNIEXPORT void JNICALL Java_com_as_imgpro_arbrands_OpenCV_setSURFParams(JNIEnv* env, jobject thiz,
				jint surfParams) {

		LOGI("surfParams : %d", surfParams);
		mSURFParams = surfParams;
	}

	JNIEXPORT jboolean JNICALL Java_com_as_imgpro_arbrands_OpenCV_setSourceImage(JNIEnv* env, jobject thiz,
			jintArray photo_data, jint width, jint height) {
		if (pImage != NULL) {
	...
	JNIEXPORT jbooleanArray JNICALL Java_com_as_imgpro_arbrands_OpenCV_getSourceImage(JNIEnv* env, jobject thiz) {
		if (pImage == NULL) {
